# just put # noqa at the end of the line and the linters will ignore the line (PEP8 check)  #noqa
"""
# ===================
# Imports
# ===================
...
def getUserID(email):
    try:
        ...
        return user.id
# ---advice from udacity review:
#  A bare except: clause will catch SystemExit and KeyboardInterrupt exceptions,# noqa
#  making it harder to interrupt a program with Control-C, and can disguise other problems.  # noqa
#  If you want to catch all exceptions that signal program errors,
...
# -------------define public function (login required decorator):
...
# ---User local permission, authentication check:
    if 'email' not in login_session:
...
def editCategory(category_id):
    # ---User local permission (authentication):
    # if 'email' not in login_session:
    # return redirect('/login')
    # -------
...
# -------user local permission
    ...
    if 'email' not in login_session or creator.id != login_session['user_id']:
        return render_template('list_public.html', items=items, category=category, creator=creator)  # noqa
    else:
        return render_template('list.html', items=items, category=category, creator=creator)  # noqa
# -------user local permission system
# if 'email' not in login_session:
#     return render_template('list_public.html', items = items, category = category)  # noqa
# else:
...
def newListItem(category_id):
    ...
    category = session.query(Category).filter_by(id=category_id).one()
    if request.method == 'POST' and request.form['name']:
        newItem = ListItem(name=request.form['name'], description=request.form['description'], category_id=category_id, user_id=category.user_id)  # noqa
        # newItem = ListItem(name = request.form['name'], description = request.form['description'], category_id = category_id)  # noqa
        session.add(newItem)
        session.commit()
...
@app.route('/category/<int:category_id>/list/<int:list_id>/edit', methods=['GET', 'POST'])  # noqa
...
# ---User local permission (authentication):
    if 'email' not in login_session:
        return render_template('editlistitem_public.html', category_id=category_id, list_id=list_id, item=editedItem)  # noqa
# ---User local permission (authentication)
...
# ------user authorization system
        ...
        if request.method == 'POST':
            ...
            return render_template('editlistitem.html', category_id=category_id, list_id=list_id, item=editedItem)  # noqa

# Delete a list item


@app.route('/category/<int:category_id>/list/<int:list_id>/delete', methods=['GET', 'POST'])  # noqa
@login_required
def deleteListItem(category_id, list_id):
    ...
    # ------user authorization system
    listToDelete = session.query(ListItem).filter_by(id=list_id).one()
    if listToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction(){alert('You are not authorized to delete this restaurant. Please create your own restaurant in order to delete.');}</script><body onload='myFunction()''>"  # noqa
    # ------user authorization system
